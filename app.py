...
def fetching_poster(movie_id):
    response = requests.get('https://api.themoviedb.org/3/movie/{}?api_key=473a074279b931c3e731a120f078715d&language=en-US'.format(movie_id))
    # the above is to hit the api and get the required poster for a given movie_id
    data = response.json()
    # data['poster_path'] it is not complete path we have do this
